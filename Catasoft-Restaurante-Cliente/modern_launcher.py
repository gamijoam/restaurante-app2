...
class ModernLauncher:
    ...
    def create_widgets(self):
        ...
        # Botón para finalizar puertos
        self.kill_ports_button = ctk.CTkButton(
            action_buttons_frame,
            text="🔌 Finalizar Puertos",
            command=self.kill_all_ports,
            font=ctk.CTkFont(size=14),
            height=40,
            fg_color="red",
            hover_color="darkred"
        )
        self.kill_ports_button.pack(side="left", fill="x", expand=True, padx=5)
    ...
    def stop_frontend(self):
        """Detener frontend"""
        # Detener proceso serve o npm si está corriendo
    ...
    def start_backend(self):
        """Iniciar backend"""
        try:
            backend_path = os.path.join(get_base_dir(), self.config["backend_path"])
    ...
    def start_static_server(self, dist_path, port):
        """Iniciar servidor HTTP para servir archivos estáticos usando el servidor embebido en Python"""
        try:
            self.log(f"🌐 Ejecutando servidor Python embebido en http://localhost:{port} (directorio: {dist_path})")
            self.start_python_static_server(dist_path, port)
        except Exception as e:
            self.log(f"❌ Error iniciando servidor Python embebido: {str(e)}")
    
    def start_python_static_server(self, dist_path, port):
        """Iniciar un servidor HTTP embebido en Python para servir archivos estáticos (soporte SPA básico)"""
        class SPARequestHandler(http.server.SimpleHTTPRequestHandler):
            def __init__(self, *args, **kwargs):
                super().__init__(*args, directory=dist_path, **kwargs)

            def send_error(self, code, message=None, explain=None):
                # Si es 404, devolver index.html para soporte SPA
                if code == 404:
        ...
        def run_server():
            with socketserver.TCPServer(("", port), SPARequestHandler) as httpd:
                self.log(f"🌐 Servidor Python embebido en http://localhost:{port} (directorio: {dist_path})")
                try:
                    httpd.serve_forever()
                except Exception as e:
                    self.log(f"❌ Error en el servidor Python embebido: {str(e)}")

        # Lanzar el servidor en un hilo aparte
        t = threading.Thread(target=run_server, daemon=True)
        t.start()
        self.frontend_running = True
        self.frontend_status.configure(text=f"🟢 Frontend: Ejecutándose (Python)")
        self.frontend_button.configure(text="🛑 Frontend")
        self.log("✅ Frontend (Python embebido) iniciado correctamente")
    
    def start_frontend(self):
        """Iniciar frontend"""
        try:
            frontend_path = os.path.join(get_base_dir(), self.config["frontend_path"])
            if os.path.exists(frontend_path):
                ...
                dist_path = os.path.join(frontend_path, "dist")
                if os.path.exists(dist_path):
                    self.log("🎨 Usando build de producción (dist/)")
                    self.log(f"🎨 Directorio: {dist_path}")
                    
                    # Iniciar servidor serve para servir archivos estáticos
                    port = self.config["frontend_port"]
                    self.start_static_server(dist_path, port)
                    
                    # Esperar un momento para que el servidor se inicie
                    time.sleep(2)
                    
                    self.frontend_running = True
                    self.frontend_status.configure(text="🟢 Frontend: Ejecutándose (Build)")
                    self.frontend_button.configure(text="🛑 Frontend")
                    self.log("✅ Frontend (build) iniciado correctamente")
                    return
                
                # Si no existe dist/, intentar con npm run dev (modo desarrollo)
                package_json = os.path.join(frontend_path, "package.json")
                ...
                if not npm_cmd:
                    self.log("❌ Error: No se encontró npm. Asegúrate de tener Node.js instalado.")
                    return
                ...
                self.frontend_running = True
                self.frontend_status.configure(text="🟢 Frontend: Ejecutándose (Dev)")
                self.frontend_button.configure(text="🛑 Frontend")
                self.log("✅ Frontend (dev) iniciado correctamente")
            else:
    ...
    def find_npm(self):
        ...
        possible_npm_paths = [
            "npm",  # En PATH
            "npm.cmd",  # En Windows
            os.path.join(os.environ.get("APPDATA", ""), "npm", "npm.cmd"),  # npm global
            os.path.join(os.environ.get("PROGRAMFILES", ""), "nodejs", "npm.cmd"),  # Node.js instalación
            os.path.join(os.environ.get("PROGRAMFILES(X86)", ""), "nodejs", "npm.cmd"),  # Node.js instalación x86
        ]
        ...
        for npm_path in possible_npm_paths:
            try:
                ...
                if result.returncode == 0:
                    self.log(f"✅ npm encontrado en: {npm_path}")
                    return npm_path
    ...
    def start_license_service(self):
        ...
        try:
            license_path = os.path.join(get_base_dir(), self.config["license_path"])
    ...
    def start_puente(self):
        ...
        try:
            puente_path = os.path.join(get_base_dir(), self.config["puente_path"])
            if os.path.exists(puente_path):
                ...
                if not node_cmd:
                    self.log("❌ Error: No se encontró node. Asegúrate de tener Node.js instalado.")
                    return
                
                self.log(f"🖨️ Ejecutando: {node_cmd} index.js")
    ...
    def find_node(self):
        ...
        possible_node_paths = [
            "node",  # En PATH
            "node.exe",  # En Windows
            os.path.join(os.environ.get("PROGRAMFILES", ""), "nodejs", "node.exe"),  # Node.js instalación
            os.path.join(os.environ.get("PROGRAMFILES(X86)", ""), "nodejs", "node.exe"),  # Node.js instalación x86
        ]
        ...
        for node_path in possible_node_paths:
            try:
                ...
                result = subprocess.run([node_path, "--version"], 
                                     ...
                                     timeout=5)
                if result.returncode == 0:
                    self.log(f"✅ node encontrado en: {node_path}")
                    return node_path
            except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
                continue
    ...
    def kill_all_ports(self):
        """Finalizar todos los procesos y puertos"""
        try:
            self.log("🔌 Finalizando todos los procesos y puertos...")
            
            # Detener todos los servicios
            self.stop_services()
            
            # Finalizar procesos específicos por puerto
            ports_to_kill = [
                self.config["backend_port"],
                self.config["frontend_port"], 
                self.config["license_port"]
            ]
            
            for port in ports_to_kill:
                self.kill_process_by_port(port)
            
            self.log("✅ Todos los procesos y puertos han sido finalizados")
            
        except Exception as e:
            self.log(f"❌ Error finalizando puertos: {str(e)}")
    
    def kill_process_by_port(self, port):
        """Finalizar proceso que usa un puerto específico"""
        try:
            import psutil
                ...
                try:
                    ...
                    for conn in connections:
                        if conn.laddr.port == port:
                            self.log(f"🔌 Finalizando proceso {proc.info['name']} (PID: {proc.info['pid']}) en puerto {port}")
                            proc.terminate()
                            proc.wait(timeout=5)  # Esperar hasta 5 segundos
                            self.log(f"✅ Proceso finalizado exitosamente")
                            return
                except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.TimeoutExpired):
                    continue
                    
        except ImportError:
            self.log("⚠️ psutil no está disponible. Usando método alternativo...")
            # Método alternativo usando netstat
            self.kill_process_by_port_alternative(port)
        except Exception as e:
            self.log(f"❌ Error finalizando proceso en puerto {port}: {str(e)}")
    
    def kill_process_by_port_alternative(self, port):
        """Método alternativo para finalizar proceso por puerto usando netstat"""
        try:
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    if f":{port}" in line and "LISTENING" in line:
                        ...
                        if len(parts) >= 5:
                            pid = parts[-1]
                            self.log(f"🔌 Encontrado proceso PID {pid} en puerto {port}")
                            
                            # Terminar proceso
                            subprocess.run(["taskkill", "/PID", pid, "/F"], 
                                         capture_output=True)
                            self.log(f"✅ Proceso PID {pid} finalizado")
                            return
                            
        except Exception as e:
            self.log(f"❌ Error en método alternativo para puerto {port}: {str(e)}")
